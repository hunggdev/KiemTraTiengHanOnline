...
async function createKoreanTest() {
  console.log(`Đang xử lý ${rawVocab.length} từ vựng...`);

  // Lấy 100 từ vựng đầu tiên
  const selectedVocab = rawVocab.slice(0, 100);

  // Tất cả các nghĩa tiếng Việt để làm pool random đáp án sai
  ...
  const questions = selectedVocab.map((item, index) => {
    ...
    return {
      ...
      score: 0.1, // 100 câu x 0.1 = 10 điểm
  ...
  // Chia thành 2 phần (mỗi phần 50 câu)
  ...
  console.log("Đang lưu bài test vào database...");
  ...
  const test = await prisma.test.create({
    data: {
      ...
      durationMin: 60,
      createdBy: "ADMIN",
      isPublished: true, // Xuất bản ngay để học sinh làm bài
      sections: {
        create: [
          {
            skill: "READING",
  ...
  console.log("✅ Tạo bài kiểm tra 100 câu thành công!");
  console.log("Test ID:", test.id);
  console.log("Title:", test.title);
  console.log("Số phần:", test.sections.length);
...
createKoreanTest()
  .catch((e) => {
    console.error("❌ Lỗi khi tạo bài test:", e);
    process.exit(1);
  })
  .finally(async () => {
